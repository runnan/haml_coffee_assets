    ...
    (function () {
      ...
      module.exports = HamlCoffee = (function () {
        ...
        function HamlCoffee(options) {
          ...
          if (this.options.placement === 'global') {
            ...
            segments = (
              ...
              .replace(/(\s|-)+/g, '_')
              .split(/\./);
            this.options.name = this.options.basename
              ? segments.pop().split(/\/|\\/).pop()
              : segments.pop();
        ...
        HamlCoffee.prototype.updateTabSize = function () {
          if (this.tabSize === 0) {
            return (this.tabSize =
              this.currentIndent - this.previousIndent);
          }
        };
          ...
          if (!this.node.isCommented()) {
            if (
              this.currentBlockLevel -
                Math.floor(this.currentBlockLevel) >
              0
            ) {
        ...
        HamlCoffee.prototype.updateCodeBlockLevel = function (node) {
          if (node instanceof Code) {
            return (this.currentCodeBlockLevel =
              node.codeBlockLevel + 1);
          } else {
        ...
        HamlCoffee.prototype.getNodeOptions = function (override) {
          ...
          return {
            ...
            codeBlockLevel:
              override.codeBlockLevel || this.currentCodeBlockLevel,
            escapeHtml:
              override.escapeHtml || this.options.escapeHtml,
            escapeAttributes:
              override.escapeAttributes ||
              this.options.escapeAttributes,
            cleanValue:
              override.cleanValue || this.options.cleanValue,
            format: override.format || this.options.format,
            hyphenateDataAttrs:
              override.hyphenateDataAttrs || this.options.format,
            preserveTags:
              override.preserveTags || this.options.preserveTags,
            selfCloseTags:
              override.selfCloseTags || this.options.selfCloseTags,
        ...
        HamlCoffee.prototype.nodeFactory = function (expression) {
          ...
          if (
            expression.match(
              /^:(escaped|preserve|css|javascript|plain|cdata|coffeescript)/
            )
          ) {
        ...
        HamlCoffee.prototype.parse = function (source) {
          ...
          this.lineNumber =
            ...
            this.tabSize =
            this.currentBlockLevel =
            this.previousBlockLevel =
              0;
          this.currentCodeBlockLevel =
            this.previousCodeBlockLevel = 0;
          this.node = null;
          this.stack = [];
          ...
          while ((line = lines.shift()) !== void 0) {
            if (this.node instanceof Filter && !this.exitFilter) {
              if (/^(\s)*$/.test(line)) {
                ...
                for (_j = 0, _len = range.length; _j < _len; _j++) {
                  ...
                  text = line.match(
                    RegExp(
                      '^\\s{' +
                        (this.node.blockLevel * tabsize + tabsize) +
                        '}(.*)'
                    )
                  );
                  if (text) {
              ...
              while (
                ...
                RegExp(
                  '^\\s{' +
                    (this.previousIndent + (this.tabSize || 2)) +
                    '}'
                ).test(lines[0]) &&
                !/^(\s*)[-=&!~.%#</]/.test(lines[0]) &&
                /([-\w]+[\w:-]*\w?)\s*=|('\w+[\w:-]*\w?')\s*=|("\w+[\w:-]*\w?")\s*=|(\w+[\w:-]*\w?):|('[-\w]+[\w:-]*\w?'):|("[-\w]+[\w:-]*\w?"):|:(\w+[\w:-]*\w?)\s*=>|:?'([-\w]+[\w:-]*\w?)'\s*=>|:?"([-\w]+[\w:-]*\w?)"\s*=>/.test(
                  lines[0]
                ) &&
                !/;\s*$/.test(lines[0])
              ) {
              ...
              while (
                ...
                RegExp(
                  '^\\s{' +
                    (this.currentIndent + (this.tabSize || 2)) +
                    '}'
                ).test(lines[0]) &&
                lines.length > 0
              ) {
        ...
        HamlCoffee.prototype.precompile = function () {
          ...
          if (code.indexOf('$fp') !== -1) {
            if (this.options.customFindAndPreserve) {
              fn +=
                '$fp = ' + this.options.customFindAndPreserve + '\n';
            } else {
              fn +=
                '$fp = (text) ->\n  text.replace /<(' +
                this.options.preserveTags.split(',').join('|') +
                ')>([^]*?)<\\/\\1>/g, (str, tag, content) ->\n    "<#{ tag }>#{ $p content }</#{ tag }>"\n';
            }
          }
          ...
          if (code.indexOf('succeed') !== -1) {
            if (this.options.customSucceed) {
              fn +=
                'succeed = (start, end, fn) => ' +
                this.options.customSucceed +
                '.call(@, start, end, fn)\n';
            } else {
              fn +=
          ...
          if (code.indexOf('precede') !== -1) {
            if (this.options.customPrecede) {
              fn +=
                'precede = (start, end, fn) => ' +
                this.options.customPrecede +
                '.call(@, start, end, fn)\n';
            } else {
              fn +=
          ...
          return (fn +=
            'return $o.join("\\n")' +
            this.convertBooleans(code) +
            this.removeEmptyIDAndClass(code) +
              ...
              switch (line.type) {
                case 'text':
                  code.push(
                    '' +
                      whitespace(line.cw) +
                      this.getBuffer(this.blockLevel) +
                      '.push "' +
                      whitespace(line.hw) +
                      line.text +
                      '"'
      ...
      module.exports = Node = (function () {
        ...
        Node.prototype.getOpener = function () {
          if (this.wsRemoval.around && this.opener.text) {
            this.opener.text =
              Node.CLEAR_WHITESPACE_LEFT + this.opener.text;
          }
        ...
        Node.prototype.getCloser = function () {
          if (this.wsRemoval.inside && this.closer.text) {
            this.closer.text =
              Node.CLEAR_WHITESPACE_LEFT + this.closer.text;
          }
        ...
        Node.prototype.markText = function (text, escape) {
          ...
          return {
            ...
            hw: this.uglify
              ? 0
              : this.blockLevel - this.codeBlockLevel,
        ...
        Node.prototype.markInsertingCode = function (
          code,
          escape,
          preserve,
          findAndPreserve
    ...
    (function () {
      module.exports = {
        ...
        indent: function (text, spaces) {
          return text.replace(
            /^(.*)$/gm,
            module.exports.whitespace(spaces) + '$1'
          );
        },
      };
    }).call(this);
    ...
    (function () {
      ...
      module.exports = Haml = (function (_super) {
        ...
        Haml.prototype.parseExpression = function (exp) {
          ...
          id = this.interpolateCodeAttribute(
            (_ref1 = tag.ids) != null ? _ref1.pop() : void 0,
            true
          );
          classes = tag.classes;
          attributes = {};
          if (tag.attributes) {
            ...
            for (key in _ref2) {
              ...
              if (key === 'id') {
                if (id) {
                  id +=
                    '_' + this.interpolateCodeAttribute(value, true);
                } else {
        ...
        Haml.prototype.parseTag = function (exp) {
          ...
          try {
            ...
            if (rest.match(/^[{([]/)) {
              ...
              for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
                ...
                if (start === rest[0]) {
                  ...
                  level = 0;
                  for (
                    pos = _j = 0, _ref3 = rest.length;
                    0 <= _ref3 ? _j <= _ref3 : _j >= _ref3;
                    pos = 0 <= _ref3 ? ++_j : --_j
                  ) {
                    ch = rest[pos];
                    if (ch === start) {
            ...
            _ref4 = [
              ...
              this.parseAttributes(rubyAttributes),
            ];
            ...
            if (
              (whitespace =
                (_ref5 = assignment.match(/^[<>]{0,2}/)) != null
                  ? _ref5[0]
                  : void 0)
            ) {
            ...
            error = _error;
            throw new Error(
              'Unable to parse tag from ' + exp + ': ' + error
            );
          }
        };
        ...
        Haml.prototype.parseAttributes = function (exp) {
          ...
          for (
            pos = _i = startPos;
            startPos <= endPos ? _i < endPos : _i > endPos;
            pos = startPos <= endPos ? ++_i : --_i
          ) {
            ch = exp[pos];
            if (ch === '(') {
              level += 1;
          ...
          for (_j = 0, _len = _ref1.length; _j < _len; _j++) {
            marker = _ref1[_j];
            exp =
              exp.substring(0, marker.start) +
              exp
                .substring(marker.start, marker.end)
                .replace(/(:|=|=>)/g, '\u0090$1') +
          ...
          while (pairs.length) {
            ...
            if (keyValue.length === 1) {
              ...
              key =
                (_ref2 = keyValue[0]) != null
                  ? _ref2.replace(/^\s+|\s+$/g, '').replace(/^:/, '')
                  : void 0;
              if ((quoted = key.match(/^("|')(.*)\1$/))) {
                key = quoted[2];
              }
              value =
                (_ref3 = keyValue[1]) != null
                  ? _ref3
                      .replace(/^\s+|[\s,]+$/g, '')
                      .replace(/\u0090/g, '')
                  : void 0;
          ...
          if (tokens.classes) {
            ...
            if (hasDynamicClass && classList.length > 1) {
              ...
              for (_i = 0, _len = classList.length; _i < _len; _i++) {
                ...
                classes +=
                  '' +
                  this.quoteAndEscapeAttributeValue(klass, true) +
                  ',';
              }
              classes =
              ...
              if (value === 'true' || value === 'false') {
                  ...
                  if (this.format === 'html5') {
                    ...
                    tagParts.push(
                      '' +
                        key +
                        '=' +
                        this.quoteAndEscapeAttributeValue(key)
                    );
                  }
                }
              } else {
                tagParts.push(
                  '' +
                    key +
                    '=' +
                    this.quoteAndEscapeAttributeValue(
                      this.interpolateCodeAttribute(value)
                    )
                );
              }
            }
        ...
        Haml.prototype.interpolateCodeAttribute = function (
          text,
          unwrap
        ) {
          var quoted;
        ...
        Haml.prototype.quoteAndEscapeAttributeValue = function (
          value,
          code
        ) {
          var escaped,
        ...
        Haml.prototype.isNotSelfClosing = function (tag) {
          return (
            this.selfCloseTags.indexOf(tag) === -1 &&
            !tag.match(/\/$/)
          );
        };
        ...
        Code.prototype.evaluate = function () {
          ...
          if (identifier === '-') {
            ...
            if (
              ...
              this.opener.code.match(/(->|=>)/)
            ) {
              return (this.closer = this.markRunningCode("  ''"));
            }
          } else if (identifier === '~') {
            if (this.escapeHtml) {
              return (this.opener = this.markInsertingCode(
                code,
                true,
                false,
                true
            ...
            if (
              ...
              this.opener = this.markInsertingCode(
                code,
                escape,
                false,
                false
              ...
              this.closer = this.markRunningCode(
                '  $buffer.join "\\n"'
              );
    ...
    (function () {
      ...
      module.exports = Filter = (function (_super) {
        ...
        Filter.prototype.evaluate = function () {
          var _ref1;
          return (this.filter =
            (_ref1 = this.expression.match(
              /:(escaped|preserve|css|javascript|coffeescript|plain|cdata|coffeescript)(.*)?/
            )) != null
              ? _ref1[1]
              : void 0);
        };
          ...
          switch (this.filter) {
            case 'escaped':
              ...
              for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
                child = _ref1[_i];
                output.push(
                  this.markText(child.render()[0].text, true)
                );
              }
              break;
            case 'preserve':
            ...
            case 'javascript':
              if (this.format === 'html5') {
                ...
                output.push(
                  this.markText("<script type='text/javascript'>")
                );
              }
              if (this.format === 'xhtml') {
                output.push(this.markText('  //<![CDATA['));
        ...
        Filter.prototype.renderFilterContent = function (
          indent,
          output,
          type
        ) {
            ...
            if (line === '') {
              ...
              switch (type) {
                case 'text':
                  for (
                    e = _k = 0;
                    0 <= empty ? _k < empty : _k > empty;
                    e = 0 <= empty ? ++_k : --_k
                  ) {
                    output.push(this.markText(''));
                  }
                  output.push(
                    this.markText('' + whitespace(indent) + line)
                  );
                  break;
                case 'run':
                  output.push(
                    this.markRunningCode('' + unescapeQuotes(line))
                  );
              }
  ...
  function (
    ...
    (function () {
      ...
      module.exports = Directive = (function (_super) {
        ...
        function Directive() {
          _ref = Directive.__super__.constructor.apply(
            this,
            arguments
          );
          return _ref;
        ...
        Directive.prototype.directives = {
          include: function (expression) {
            ...
            try {
              ...
              e = _error;
              throw new Error(
                'Failed to parse the include directive from ' +
                  expression
              );
            }
            if (!context) {
              ...
              switch (this.placement) {
                ...
                case 'standalone':
                  if (
                    ...
                    throw new Error(
                      'Include directive not available in the Browser when placement is standalone.'
                    );
                  } else {
                    try {
                ...
                default:
                  throw new Error(
                    'Include directive not available when placement is ' +
                      this.placement
                  );
              }
            }.call(this);
        ...
        Directive.prototype.evaluate = function () {
          ...
          try {
            ...
            e = _error;
            throw new Error(
              'Unable to recognize directive from ' + this.expression
            );
          }
        ...
        __express: function (filename, options, callback) {
          ...
          try {
            if (options.cache && __expressCache[filename]) {
              return callback(
                null,
                __expressCache[filename](options)
              );
            } else {
              options.filename = filename;
              source = fs.readFileSync(filename, 'utf8');
              if (options.cache) {
                __expressCache[filename] = module.exports.compile(
                  source,
                  options
                );
                return callback(
                ...
                return callback(
                  null,
                  module.exports.compile(source, options)(options)
                );
              }
            }
          } catch (_error) {
